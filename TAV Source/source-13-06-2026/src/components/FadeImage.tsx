...
interface FadeImageProps extends React.ImgHTMLAttributes<HTMLImageElement> {
  ...
  /** If true, load immediately without deferring to idle */
  priority?: boolean;
  aspectRatio?: string;
}
...
const FadeImage = ({
  ...
  useEffect(() => {
    if (priority) {
      const raf = requestAnimationFrame(() => setActiveSrc(src));
  ...
  useEffect(() => {
    ...
    if (priority) {
      ...
      const t = setTimeout(loadFull, 50);
      return () => clearTimeout(t);
    }
  }, [loadFullRes, activeSrc, src, priority]);
