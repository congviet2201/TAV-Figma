...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (type === "image") {
      // Defer setting videoLoaded until FadeImage is ready (via onLoad)
      // Remove manual preloading - let FadeImage handle it with priority
    }
  }, [dataApi]);
  ...
  const renderMedia = () => {
    ...
    if (!picture) {
      return <div className="products-lazy-video-wrap home-hero-wrap" style={{ aspectRatio: isMobile ? undefined : "16/9", width: "100%", height: isMobile ? "100dvh" : undefined }} />;
    }
    ...
    if (type === "video") {
      return (
        <LazyVideo
          ...
          wrapStyle={{ width: "100%", height: undefined }}
    ...
    if (type === "image") {
      return (
        <FadeImage
          ...
          loadFullRes={false}
          aspectRatio={isMobile ? undefined : "16/9"}
          wrapStyle={{ width: "100%", height: isMobile ? "100dvh" : undefined }}
